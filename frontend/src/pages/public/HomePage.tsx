...
function HeroSection() {
  ...
  const techStack = [
    ...
    { label: "Docker",      color: "text-blue-300   border-blue-400/30   bg-blue-400/10"   },
  ];
    ...
    <section className="relative flex items-center overflow-hidden bg-gray-950 min-h-[90vh]">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-12">
          ...
          <div className="flex flex-col justify-center order-2 lg:order-1">
            ...
            <div className="flex items-center gap-2 mb-7 w-fit">
              <span className="flex h-7 items-center gap-2 px-3.5 py-1 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 text-xs font-semibold">
                <span className={cn(
                  ...
                  settings.open_to_work ? "bg-green-400 animate-pulse-dot" : "bg-gray-500"
                )} />
                {settings.availability_text}
              </span>
              <span className="text-gray-600 text-xs hidden sm:block">· {settings.location}</span>
            </div>
            ...
            <h1 className="text-4xl sm:text-5xl xl:text-6xl font-black text-white leading-[1.08] tracking-tight mb-5">
              ...
              <span className="relative inline-block">
                <span className="gradient-text">{settings.full_name}</span>
                <svg className="absolute -bottom-2 left-0 w-full" height="6" viewBox="0 0 200 6" preserveAspectRatio="none">
                  <path d="M0,5 Q50,0 100,4 Q150,8 200,3" stroke="url(#underlineGrad)" strokeWidth="2.5" fill="none" strokeLinecap="round" />
                  <defs>
            ...
            <p className="text-gray-400 text-base sm:text-lg leading-relaxed mb-7 max-w-lg">
              {settings.bio_short || "I design and ship production-grade web applications — clean APIs, robust backends, and polished frontends."}
            </p>

            {/* Tech pills */}
            <div className="flex flex-wrap gap-2 mb-8">
              {techStack.map((tech) => (
                <span key={tech.label} className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${tech.color} transition-all duration-200 hover:scale-105`}>
                  {tech.label}
                </span>
              ))}
            </div>
          ...
          <div className="order-1 lg:order-2 flex items-center justify-center lg:justify-end relative">
            ...
            <div className="relative w-full max-w-[520px] lg:max-w-none lg:w-[110%]">
              ...
              <div className="absolute -left-4 top-8 bg-gray-900/95 backdrop-blur-sm border border-gray-800 rounded-2xl px-4 py-3 shadow-xl hidden sm:block">
                <p className="text-2xl font-black text-white">{settings.stat_experience}</p>
              ...
              <div className="absolute right-8 top-4 bg-gray-900/95 backdrop-blur-sm border border-blue-800/50 rounded-xl px-3 py-2 shadow-xl hidden md:block">
                <p className="text-xs text-blue-400 font-bold">{settings.stat_projects} Projects</p>
        ...
        <div className="flex justify-center mt-4 lg:mt-0">
          <a href="#about" className="flex flex-col items-center gap-2 text-gray-600 hover:text-gray-400 transition-colors duration-200 group">
    ...
    <div className="relative bg-gray-950 -mb-1">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="flex items-center gap-6">
          ...
          <span className="text-[10px] font-bold tracking-widest uppercase text-gray-600">About</span>
...
function AboutSection() {
  ...
  const stats = [
    { label: "Years Experience", value: settings.stat_experience  },
    { label: "Projects Shipped", value: settings.stat_projects    },
    { label: "Technologies",     value: settings.stat_technologies },
    { label: "Open Source PRs",  value: settings.stat_open_source },
  ];
  ...
  const bio1 = settings.bio_long  || "I'm a full-stack engineer with over 5 years of experience building scalable web applications. I specialise in Python/Django backends and React frontends — with a strong focus on clean architecture, testability, and developer experience.";
  const bio2 = settings.bio_long_2 || "I've worked across startups and mid-size companies, shipping products from zero to production. I care about code quality, API design, and building systems that teams can extend confidently for years.";
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative order-2 lg:order-1">
            <div className="relative w-full max-w-md mx-auto">
                ...
                <div className="text-center mb-6">
                  {settings.profile_image ? (
                    ...
                    <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-blue-600 to-cyan-500 flex items-center justify-center text-white text-3xl font-black mx-auto mb-4 shadow-xl shadow-blue-500/30">
                      {settings.full_name.split(" ").map((n) => n[0]).join("").slice(0, 2)}
                    </div>
                  )}
                  <h3 className="font-black text-gray-900 text-xl">{settings.full_name}</h3>
                  <p className="text-blue-600 text-sm font-semibold">{settings.tagline}</p>
                  <div className="flex items-center justify-center gap-1.5 mt-2">
                    <span className={cn(
                      "w-2 h-2 rounded-full",
                      settings.open_to_work ? "bg-green-500 animate-pulse-dot" : "bg-gray-400"
                    )} />
                    <span className="text-xs text-gray-400 font-medium">
                      {settings.open_to_work ? "Available Now" : "Not Currently Available"}
                    </span>
                  </div>
                </div>
                <div className="space-y-2.5">
                  {highlights.map(({ icon: Icon, label, desc }) => (
                    <div key={label} className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100 hover:border-blue-200 hover:bg-blue-50/50 transition-all duration-200">
              ...
              <div className="absolute -top-5 -right-5 bg-white rounded-2xl shadow-xl border border-gray-100 px-4 py-3">
                <p className="text-2xl font-black text-gray-900">{settings.stat_experience}</p>
          ...
          <div className="order-1 lg:order-2">
            <p className="text-blue-600 font-bold text-xs uppercase tracking-widest mb-3">About Me</p>
            <h2 className="text-4xl font-black text-gray-900 leading-[1.1] mb-5">
              Turning complex problems<br />into{" "}
              <span className="gradient-text">elegant solutions</span>
            </h2>
            <p className="text-gray-600 leading-relaxed mb-4 text-[15px]">{bio1}</p>
            {bio2 && <p className="text-gray-600 leading-relaxed mb-8 text-[15px]">{bio2}</p>}
            <div className="flex items-center gap-2 text-sm mb-8 font-semibold text-gray-500">
              ...
              {settings.location} · Open to Remote Roles
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {stats.map(({ label, value }) => (
                <div key={label} className="text-center p-4 rounded-2xl bg-white border border-gray-200 shadow-sm hover:border-blue-300 hover:shadow-md hover:shadow-blue-100 transition-all duration-200 cursor-default">
...
function FeaturedProjectsSection({
  projects, loading,
}: { projects: ProjectSummary[]; loading: boolean }) {
  return (
    <section id="projects" className="py-24 bg-gray-50 relative">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div className="flex items-end justify-between mb-12">
          <div>
            <p className="text-blue-600 font-semibold text-xs uppercase tracking-widest mb-3">My Work</p>
            <h2 className="text-4xl font-black text-gray-900">Featured Projects</h2>
          </div>
          <Link to="/projects" className="hidden sm:flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700 group">
            ...
            : projects.map((project) => (
                <Link key={project.id} to={`/projects/${project.slug}`}
                  ...
                  <div className="relative h-48 overflow-hidden bg-gradient-to-br from-blue-50 to-indigo-100 shrink-0">
                    {project.cover_image ? (
                      <img src={project.cover_image} alt={project.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-out" />
                    ) : (
                    ...
                    {project.is_featured && (
                      <div className="absolute top-3 right-3">
                        <span className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-yellow-400 text-yellow-900 text-xs font-bold shadow-sm">★ Featured</span>
                      </div>
                    )}
                  </div>
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <h3 className="font-bold text-gray-900 text-lg mb-1.5 group-hover:text-blue-600 transition-colors duration-200">{project.title}</h3>
                    <p className="text-gray-500 text-sm leading-relaxed mb-4 line-clamp-2 flex-1">{project.summary}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {project.technologies.slice(0, 4).map((tech) => (
                        <span key={tech.id} className="px-2.5 py-1 rounded-full bg-gray-100 text-gray-600 text-xs font-medium">{tech.name}</span>
                      ))}
                      {project.technologies.length > 4 && (
                        <span className="px-2.5 py-1 rounded-full bg-gray-100 text-gray-400 text-xs font-medium">+{project.technologies.length - 4}</span>
                      )}
                    </div>
                  </div>
                  <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between">
                    <span className="text-xs text-gray-400 font-medium">{project.project_start ? formatDateShort(project.project_start) : ""}</span>
                    <span className="text-xs font-semibold text-blue-600 flex items-center gap-1 group-hover:gap-2 transition-all">View Project <ArrowRight size={12} /></span>
                  </div>
                </Link>
              ))}
        </div>
...
// ---------------------------------------------------------------------------
// SKILLS — Dark background (breaks white monotony)
// ---------------------------------------------------------------------------
    ...
    <section id="skills" className="py-24 bg-gray-950 relative">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        <div className="text-center mb-14">
          <p className="text-blue-400 font-semibold text-xs uppercase tracking-widest mb-3">Tech Arsenal</p>
            ...
            <div key={cat}>
              ...
              <div className="flex flex-wrap gap-3">
                {skills.filter((s) => s.category === cat).map((skill) => (
                  <div key={skill.id}
                    className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-gray-800/60 border border-gray-700/60 hover:border-blue-500/40 hover:bg-gray-800 hover:shadow-sm hover:shadow-blue-500/10 transition-all duration-200 cursor-default group">
                    ...
                    <div className="flex gap-0.5">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <div key={i} className={cn(
                          ...
                          i < skill.proficiency ? "bg-blue-500 group-hover:bg-blue-400" : "bg-gray-700"
                        )} />
                      ))}
                    </div>
                  </div>
      ...
      {/* Bottom wave dark to light */}
      <div className="absolute bottom-0 left-0 right-0 overflow-hidden leading-none">
        <svg viewBox="0 0 1440 60" className="w-full fill-gray-50">
          <path d="M0,60 Q720,20 1440,60 L1440,0 L0,0 Z" />
        </svg>
      </div>
    </section>
  );
  ...
  return (
    <section id="experience" className="py-24 bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          <p className="text-blue-600 font-semibold text-xs uppercase tracking-widest mb-3">Work History</p>
            ...
            {experience.map((exp, i) => (
              <div key={exp.id} className="relative flex gap-6 sm:gap-8">
                <div className="relative z-10 shrink-0">
                  <div className={cn(
                    "w-12 sm:w-16 h-12 sm:h-16 rounded-2xl flex items-center justify-center font-black text-sm shadow-md",
                    i === 0 ? "bg-gradient-to-br from-blue-600 to-cyan-500 text-white shadow-blue-500/30" : "bg-white text-gray-600 border-2 border-gray-200"
                  )}>
                ...
                <div className="flex-1 bg-white rounded-2xl border border-gray-200/80 p-5 sm:p-6 hover:shadow-md hover:border-blue-100 transition-all duration-200">
                  ...
                  <p className="text-gray-600 leading-relaxed text-sm whitespace-pre-line">{exp.description}</p>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <section id="blog" className="py-24 bg-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between mb-12">
          <div>
            <p className="text-blue-600 font-semibold text-xs uppercase tracking-widest mb-3">Writing</p>
            <h2 className="text-4xl font-black text-gray-900">Latest Posts</h2>
          </div>
          <Link to="/blog" className="hidden sm:flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700 group">
            All Posts <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
            ...
            : posts.map((post) => (
                <Link key={post.id} to={`/blog/${post.slug}`}
                  ...
                  <div className="h-44 bg-gradient-to-br from-purple-50 to-blue-50 overflow-hidden relative">
                    {post.cover_image ? (
                      <img src={post.cover_image} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-out" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center">
                  ...
                  <div className="p-5">
                    {post.category && <Badge variant="info" className="mb-2.5 text-xs">{post.category.name}</Badge>}
                    <h3 className="font-bold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors duration-200 line-clamp-2 leading-snug">{post.title}</h3>
  ...
  return (
    <section className="py-24 bg-gray-950 relative overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div className="text-center mb-14">
          <p className="text-blue-400 font-semibold text-xs uppercase tracking-widest mb-3">Social Proof</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {testimonials.map((t, i) => (
            <div key={t.id} className={cn(
              ...
              i === 0 ? "bg-blue-600 border-blue-500" : "bg-gray-900 border-gray-800 hover:bg-gray-800/80"
            )}>
              <div className={cn("text-5xl font-black leading-none mb-4 -mt-1", i === 0 ? "text-blue-300/60" : "text-gray-700")}>"</div>
              <p className={cn("text-sm leading-relaxed mb-6", i === 0 ? "text-blue-50" : "text-gray-300")}>{t.testimonial}</p>
              <div className="flex items-center gap-3">
                {t.photo ? (
                  <img src={t.photo} alt={t.name} className="w-10 h-10 rounded-full object-cover ring-2 ring-white/20" />
                ) : (
                  <div className={cn("w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0", i === 0 ? "bg-blue-500 text-white" : "bg-gray-700 text-gray-300")}>
                    {t.name[0]}
                  </div>
                )}
                <div>
                  <p className="font-semibold text-sm text-white">{t.name}</p>
                  <p className={cn("text-xs", i === 0 ? "text-blue-200" : "text-gray-500")}>{t.role} · {t.company}</p>
                </div>
              </div>
            </div>
          ))}
    ...
    <section id="contact" className="py-24 bg-white relative">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div>
            <p className="text-blue-600 font-semibold text-xs uppercase tracking-widest mb-3">Get In Touch</p>
          ...
          <div>
            {sent ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-16">
                ...
                <p className="text-gray-500 mb-6 max-w-xs">Thanks for reaching out. I'll get back to you within 24 hours.</p>
                <button onClick={() => setSent(false)} className="text-blue-600 text-sm font-semibold hover:underline">Send another message</button>
              </div>
            ) : (
              <div className="bg-gray-50/80 rounded-2xl border border-gray-200/80 p-6 sm:p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <button type="submit" disabled={sending}
                    className="w-full flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 text-white text-sm font-semibold shadow-md shadow-blue-500/25 hover:from-blue-700 hover:to-blue-600 transition-all duration-200 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed">
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    testimonialsService.list({ featured: true }).then((d) => setTestimonials(d.slice(0, 3)));
